...
<div class="max-w-2xl bg-white rounded-lg shadow p-6">
    <form method="POST" action="{{ route('master.customer.update', $customer->Customer_ID) }}">
        @csrf @method('PUT')
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <input type="text" name="Kontak" value="{{ old('Kontak', $customer->Kontak) }}" required 
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Alamat</label>
                <textarea name="Alamat" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('Alamat', $customer->Alamat) }}</textarea>
            </div>
        </div>
